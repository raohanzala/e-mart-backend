  ...
  try {
    ...
    if (req.file) {
      ...
      const processedBuffer = await sharp(req.file.path)
        .resize(500) // Resize to 500px width for categories
        .toBuffer();
...
const editCategory = async (req, res) => {
  try {
    ...
    let imageUrl = category.image; // Keep the old image if no new file is uploaded
    ...
    const updatedData = {
      ...
      ...(imageUrl !== category.image && { image: imageUrl }),
      ...(isFeatured !== undefined && { isFeatured: isFeatured === 'true' || isFeatured === true }),
      ...(parsedSubCategories !== undefined && { subCategories: parsedSubCategories })
    };
